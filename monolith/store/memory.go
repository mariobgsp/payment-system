...
func (m *MemoryStore) GetUserDetail(_ context.Context, username string) (*StoreUser, error) {
	if u, ok := demoUsers[username]; ok { cp:=*u; return &cp, nil }
	return nil, errors.New("not found")
}
func (m *MemoryStore) GetAllProducts(_ context.Context) ([]Product, error) { return append([]Product(nil), demoProducts...), nil }
func (m *MemoryStore) GetSpecialProducts(_ context.Context, special bool) ([]Product, error) {
	var out []Product
	for _, p := range demoProducts { if p.SpecialProduct==special { out=append(out,p) } }
	return out, nil
}
func (m *MemoryStore) GetSingleProduct(_ context.Context, code string) (*Product, error) {
	for _, p := range demoProducts { if p.ProductCode==code { cp:=p; return &cp, nil } }
	return nil, errors.New("not found")
}
