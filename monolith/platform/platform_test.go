...
func TestRequest_Ok_Fail(t *testing.T) {
	...
	if req.RequestID != "req-123" { t.Fatalf("want req-123 got %s", req.RequestID) }
	if req.Channel != "WEB" { t.Fatalf("want WEB got %s", req.Channel) }
	resp := p.Ok(req, map[string]string{"ok": "1"})
	if resp.Status != 200 { t.Fatalf("want 200") }
	if resp.Headers["x-request-id"] != "req-123" { t.Fatal("header missing") }
	ae := lifecycle.NewBadRequest("bad")
	resp2 := p.Fail(req, ae)
	if resp2.Status != 400 { t.Fatalf("want 400 got %d", resp2.Status) }
	// generic error → 500
	resp3 := p.Fail(req, &testErr{"boom"})
	if resp3.Status != 500 { t.Fatal("want 500") }
	// publish inserts outbox
	if err := p.Publish(context.Background(), req, resp); err != nil { t.Fatalf("publish %v", err) }
	list, _ := mem.ListUnprocessedOutbox(context.Background(), 10)
	if len(list) == 0 { t.Fatal("want outbox") }
}
...
func TestRequest_Jakarta(t *testing.T) {
	...
	if req.RequestAt != "2026-08-25 17:00:00" { t.Fatalf("want Jakarta time got %s", req.RequestAt) }
}
