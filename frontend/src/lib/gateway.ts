...
import type { ApiEnvelope, OrderResult, PaymentResult, Product, ProductTrx, UserDetail } from "./types";

// ponytail: single BASE hides MS_ORDER_URL / MS_PAYMENT_URL split (now monolith:8085) — collapse to one env
const BASE = process.env.MS_ORDER_URL ?? process.env.MS_PAYMENT_URL ?? "http://localhost:8080";
...
function normalizePaymentResult(raw: unknown): PaymentResult {
  ...
  const url = (r.CheckoutUrl as string) ?? (r.checkoutUrl as string) ?? (r.checkout_url as string) ?? (r.CheckoutURL as string) ?? "";
...
export class HttpGateway implements Gateway {
  ...
  payment = {
    ...
    refund: async (transactionId: string, username: string, token?: string): Promise<unknown> => {
...
export class FakeGateway implements Gateway {
  ...
  order = {
    create: async (cmd: { productCode: string; productName: string; amount: number; price: number; username: string }, _token?: string): Promise<OrderResult> => {
      ...
      this.orders[id] = {
        ...
        userId: "b2xrasd", productName: cmd.productName, amount: cmd.amount, price: cmd.price, priceCharge: cmd.price * cmd.amount,
        productCode: cmd.productCode, param1: null, param2: null, sysUpdateDate: new Date().toISOString(), paymentDate: "", discountEnabled: false, discount: 0,
      };
  ...
  _seedPaid(id: string) {
    ...
    if (o) { o.paymentStatus = "SUCCESS"; o.orderStatus = "PUBLISHED"; }
  }
}
