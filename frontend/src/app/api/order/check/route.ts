...
export async function GET(req: NextRequest) {
  ...
  const url = new URL(`${MS_ORDER_URL}/ms/api/v1/order/${encodeURIComponent(transactionId)}/check`);
  ...
  try {
    ...
    if (!res.ok || body.code !== "00") {
      return toEnvelope(false, null, body.message ?? "transaction not found", 404);
    }
