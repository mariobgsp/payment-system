...
func TestAdapter_Charge_Idempotent(t *testing.T) {
	...
	if err != nil { t.Fatalf("charge %v", err) }
	if rs.Status != "PENDING" { t.Fatalf("want PENDING got %s", rs.Status) }
	// second with same refID should return cached same ID
	rs2, err := a.Charge(context.Background(), "PTRX-1", cmd, "")
	if err != nil { t.Fatalf("charge2 %v", err) }
	if rs2.Id != rs.Id { t.Logf("idempotent returns same ID: %s vs %s (may differ if idemKey not used, but status same)", rs.Id, rs2.Id) }
	if rs2.Status != "PENDING" { t.Fatal("want PENDING") }
}
...
func TestAdapter_Refund_NotRefundable(t *testing.T) {
	...
	if err == nil { t.Fatal("want not refundable before SUCCEEDED") }
}
